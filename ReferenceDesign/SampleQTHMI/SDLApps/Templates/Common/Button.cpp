...
void CButton::SetPadding(int iPadding)
{
    m_pMainLayout->setContentsMargins(iPadding, iPadding, iPadding, iPadding);
}

void CButton::SetPadding(int iLeft, int iTop, int iRight, int iBottom)
{
...
int CButton::GetTextWidth() {
    QMargins margin = m_pMainLayout->contentsMargins();

    //  return m_bLeftIcon ? (width() - height() * 0.8) : (width());
    return m_bLeftIcon ? ((width()-(margin.left()+margin.right())) - height() * 0.8) : (width()-(margin.left()+margin.right()));
}
