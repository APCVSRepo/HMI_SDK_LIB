...
TEST_F(CDeviceListViewTest,onDeviceSelected_UpdateDeviceList)
{
    ...
    ON_CALL(appListMock,getDeviceList(_)).WillByDefault(Invoke(&appListMock.DelegateGetAppList(),&AppList::getDeviceList));
    EXPECT_CALL(appListMock,getDeviceList(_)).Times(AtLeast(1));
    appListMock.getDeviceList(DeviceList);

    EXPECT_EQ(2,DeviceList.size());
    EXPECT_EQ("Jerry's Phone",DeviceList.at(0).name);
    EXPECT_EQ("XT910",DeviceList.at(1).name);
}
